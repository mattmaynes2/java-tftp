...
public class ErrorSimulator extends Controller implements SimulationEventListener{
    ...
    private static final String BLOCKNUM_COMMAND = "bnum";
    ...
    public ErrorSimulator(String[] commandLineArgs) throws SocketException  {
        ...
        this.interpreter.addCommand(BLOCKNUM_COMMAND);
    ...
    public void usage() {
        ...
        System.out.println("    bnum          <type> <packetNum> <opCode>		Changes the block number of a specified packet");
        ...
        try{
            switch (command.getToken()){
                ...
                case OPCODE_COMMAND:
                    ...
                    break;
                case BLOCKNUM_COMMAND:
                    this.changeBlockNumSimulation(command.getArguments());
                    break;
    ...
    private void changeOpcodeSimulation(ArrayList<String> args) {
        ...
        if(args.size() != 3) {
            this.cli.message("Incorrect number of parameters for op.  Format is op <type> <packetNum> <opCode>");
            return;
        }
    ...
    /**
     * Change the block number of a packet
     * @param args  Will contain the packet type to change, its packet number, and the opcode to change it to
     */
    private void changeBlockNumSimulation(ArrayList<String> args) {
        //args must be size of three
        if(args.size() != 3) {
            this.cli.message("Incorrect number of parameters for bnum.  Format is bnum <type> <packetNum> <blockNum>");
            return;
        }

        //get the blockNum
        String blockNum = args.get(2);

        int packetNum = verifyNum(args.get(1), 1);
        if(packetNum >= 0 && packetNum < HIGHEST_PACKET) {
            //Parse out the opcode into bytes
            short blockNumShort = (short)verifyNum(blockNum, Short.MIN_VALUE);
            if(Short.MIN_VALUE < blockNumShort && blockNumShort < Short.MAX_VALUE) {
                ByteArrayOutputStream out = new ByteArrayOutputStream();
                ByteBuffer b = ByteBuffer.allocate(2);
                b.putShort(blockNumShort);
                ...
                PacketModifier modifier = new PacketModifier();
                modifier.setBlockNum(out.toByteArray());
                try {
                    SimulatorStream stream = SimulatorStreamFactory.createSimulationStream(SimulationTypes.REPLACE_PACKET, args.get(0), packetNum,  modifier);
                    recieveListener.setConfiguration(stream);
                } catch (SocketException e) {
                ...
                this.cli.message("Packet " + packetNum + " will now have its block number changed to " + blockNumShort);
            }
            else {
                this.cli.message("OpCode out of bounds.  -32727 < opcode < 32728");
            }
